...
import { deleteTestimonioAction } from "@/lib/actions/testimonios";

export default function DeleteTestimonioButton({ id }: { id: string }) {
  ...
  async function handleClick() {
    ...
    if (phase === "confirm") {
      setPhase("loading");
      try {
        await deleteTestimonioAction(id);
      } catch {
        setPhase("idle");
      }
    }
  ...
  const label =
    phase === "loading" ? "Eliminando…" : phase === "confirm" ? "¿Confirmar?" : "Eliminar";
